...
totp_key = "TZARUR26ZRWDFVBFVOT4FMZ3CG7VSXXY"
# Fyers Client ID (ex., "TK01248")
username = "YP10718"
# four-digit PIN
pin = 2320
# App ID of the created app (ex., "L9NY305RTW-100")
client_id = "W1CZBUOF5Z-100"
# Secret ID of the created app
secret_key = "PAE6CHNYWR"
# Redircet URL you entered while creating the app (ex., "https://trade.fyers.in/api-login/redirect-uri/index.html")
...
def get_token():
    ...
    r1 = s.post(
        "https://api-t2.fyers.in/vagator/v2/send_login_otp_v2", data=data1)
    ...
    r4 = s.post("https://api.fyers.in/api/v2/token",
                headers=headers, data=data4)
    print(f"r4.status_code: {r4.status_code}")
    print(f"r4.text: {r4.text}")
    # assert r4.status_code == 308, f"Error in r4:\n {r4.json()}"

    # parsed = urlparse(r4.json()["Url"])
    auth_code = parse_qs(parsed.query)["auth_code"][0]
    assert r4.status_code == 200, f"Error in r4:\n {r4.json()}"
...
def get_profile(token):
    fyers = fyersModel.FyersModel(
        client_id=client_id, token=token, log_path=os.getcwd())
